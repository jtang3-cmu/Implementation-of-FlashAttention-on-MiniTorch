...
def logsumexp(input: Tensor, dim: int) -> Tensor:
    ...
    input_sub = input - input_max
    exp_sum = input_sub.exp().sum(dim=dim)
    log_exp_sum = exp_sum.log()
    return log_exp_sum + input_max
    ### END YOUR SOLUTION
...
def softmax_loss(logits: Tensor, target: Tensor) -> Tensor:
    ...
    # Subtract logsumexp from logits to prevent overflow
    logits_prob = logsumexp(logits,dim=1) - logits

    # Convert class index to one hot encoding
    one_hot_encoding_targets = one_hot(target, logits.shape[1])
    
    # Compute the modified loss 
    result = (one_hot_encoding_targets * logits_prob).sum(dim=1)
