...
class Attn_Softmax(Function):
    ...
    def forward(ctx: Context, inp: Tensor, mask: Tensor) -> Tensor:
        ...
        # Save softmax output and mask for backward computation.
        # The mask is stored to retrieve its shape for returning zero gradients.
    ...
    def backward(ctx: Context, out_grad: Tensor) -> Tuple[Tensor, Tensor]:
        ...
        """
        # Retrieve saved softmax output and mask from the context
        ...
        # The mask does not require gradients, so return a zero tensor of the same shape
...
class LayerNorm(Function):
    ...
    def forward(ctx: Context, inp: Tensor, gamma: Tensor, beta: Tensor) -> Tensor:
        ...
        # Save input, gamma, beta, variance, and mean for backward computation
    ...
    def backward(ctx: Context, out_grad: Tensor) -> Tuple[Tensor, Tensor, Tensor]:
        ...
        # Compute gradient of the input tensor
        grad_input = inp.f.layernorm_bw(out_grad, inp, gamma, beta, var, mean)
        
        # Expand mean and variance dimensions to match the input shape (batch_size, hidden_dim)
        mean_expanded = mean.view(mean.shape[0], 1)
        var_expanded = var.view(var.shape[0], 1)
        
        # Compute normalized input: normalized = (inp - mean) / sqrt(var)
        # Use var ** (-0.5) instead of rsqrt(var) for numerical stability
        normalized = (inp - mean_expanded) * (var_expanded ** (-0.5))
        
        # Compute gradients for gamma and beta
        grad_gamma = (out_grad * normalized).sum(dim=0) # Gradient for gamma (scale parameter)
        grad_beta = out_grad.sum(dim=0) # Gradient for beta (shift parameter)
