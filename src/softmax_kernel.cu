...
void launch_attn_softmax_bw(float *out_grad,
  ...
  // Compute the size of data to be processed 
  // (assuming out_grad and soft_inp are both float arrays of size rows * softmax_len)
  ...
  // Launch the appropriate CUDA kernel based on softmax_len
  // The template parameter ITERATIONS is determined dynamically
  if (softmax_len <= 32) {
  ...
  // Ensure kernel execution completes before moving forward
  cudaDeviceSynchronize();

  // Copy the results from the device back to the host (overwriting out_grad)
  cudaMemcpy(out_grad, d_out_grad, data_size, cudaMemcpyDeviceToHost);

  // Free allocated memory on the GPU device
  cudaFree(d_out_grad);
  cudaFree(d_soft_inp);
